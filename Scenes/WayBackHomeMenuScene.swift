...
class WayBackHomeMenuScene : ParentScene {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if node.name == "1 PLAYER" {
            ...
            wayBackHomeGameScene.backScene = self
            wayBackHomeGameScene.numberPlayer = .OnePlayer
            ...
            let wayBackHomeGameScene = WayBackHomeGameScene(size: self.size)
            wayBackHomeGameScene.backScene = self
            wayBackHomeGameScene.numberPlayer = .TwoPlayer
            wayBackHomeGameScene.scaleMode = .aspectFill
            self.scene!.view?.presentScene(wayBackHomeGameScene, transition: transition)
