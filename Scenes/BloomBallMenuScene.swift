...
class BloomBallMenuScene : ParentScene {
    
    override func didMove(to view: SKView) {
        let titles = ["1 PLAYER", "2 PLAYERS", "BACK"]
        setHeader(withName: "Bloom Ball", andBackground: "header_background", width: frame.width * 0.6 ,  position: CGPoint(x: self.frame.midX, y: self.frame.maxY * 0.9))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if node.name == "1 PLAYER" {
            ...
            bloomBallGameScene.numberPlayer = .OnePlayer
            ...
            let bloomBallGameScene = BloomBallGameScene(size: self.size)
            bloomBallGameScene.backScene = self
            bloomBallGameScene.numberPlayer = .TwoPlayer
            bloomBallGameScene.scaleMode = .aspectFill
            self.scene!.view?.presentScene(bloomBallGameScene, transition: transition)
